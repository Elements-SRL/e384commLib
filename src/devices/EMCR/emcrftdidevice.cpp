...
static const std::vector <std::vector <uint32_t> > deviceTupleMapping = {
    ...
    {DeviceVersionE4p, DeviceSubversionEL07Ex8Patch_artix7_PCBV03, 1, DeviceE8PPatchEL07E_artix7_PCBV03_FW1},           //  10, 24,  1 : VC-CC device with 8 channels (EL07C) (FPGA artix7) PCB V02. */
...
ErrorCodes_t EmcrFtdiDevice::connectDevice(std::string deviceId, MessageDispatcher * &messageDispatcher, std::string fwPath) {
    ...
    case DeviceE8PPatchEL07E_artix7_PCBV03_FW1:
        messageDispatcher = new Emcr8PatchClamp_EL07e_artix7_PCBV03_fw_v01(deviceId);
        break;
...
uint32_t EmcrFtdiDevice::readDataFromDevice() {
    ...
    FT_STATUS ftRet = Ftd2xxWrapper::FTW_GetQueueStatus(* ftdiRxHandle, &ftdiQueuedBytes);
    if (ftRet != FT_OK) {
        return 0;
    }
...
bool EmcrFtdiDevice::writeToBulkOut(uint32_t * buffer) {
    ...
    while (!txDataSent && (writeTries++ < EMF_MAX_WRITE_TRIES)) { /*! \todo FCON prevedere un modo per notificare ad alto livello e all'utente */
        ftRet = Ftd2xxWrapper::FTW_Write(* ftdiTxHandle, (LPVOID)buffer, bytesToWrite, &ftdiWrittenBytes);
