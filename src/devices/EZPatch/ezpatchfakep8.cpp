...
ErrorCodes_t EZPatchFakeP8::connect(std::string fwPath) {
    if (connected) {
    ...
    rxThread = std::thread(&EZPatchFakeP8::readAndParseMessagesForGenerator, this);
    txThread = std::thread(&EZPatchFakeP8::unwrapAndSendMessagesForGenerator, this);
    gnThread = std::thread(&EZPatchFakeP8::generateData, this);
    satThread = std::thread(&EZPatchFakeP8::saturationFromGenerator, this);
...
ErrorCodes_t EZPatchFakeP8::disconnect() {
    if (!connected) {
...
void EZPatchFakeP8::ackFromGenerator(uint16_t hb) {
...
void EZPatchFakeP8::saturationFromGenerator() {
...
ErrorCodes_t EZPatchFakeP8::setVCCurrentRange(uint16_t currentRangeIdx, bool applyFlag) {
    ErrorCodes_t ret;
...
ErrorCodes_t EZPatchFakeP8::setCCCurrentRange(uint16_t currentRangeIdx, bool applyFlag) {
    ErrorCodes_t ret;
...
ErrorCodes_t EZPatchFakeP8::setSamplingRate(uint16_t samplingRateIdx, bool applyFlag) {
    ErrorCodes_t ret;
...
ErrorCodes_t EZPatchFakeP8::resetFpga() {
    std::unique_lock <std::mutex> genRxMutexLock(genRxMutex);
    std::unique_lock <std::mutex> rxMutexLock(rxMutex);
    while (rxMsgBufferReadLength >= EZP_RX_MSG_BUFFER_SIZE) { /*! \todo FCON bloccare la ricezione potrebbe essere controproducente */
        txMsgBufferNotFull.wait(rxMutexLock);
...
void EZPatchFakeP8::readAndParseMessagesForGenerator() {
    parsingFlag = true;
...
void EZPatchFakeP8::unwrapAndSendMessagesForGenerator() {
...
void EZPatchFakeP8::generateData() {
    std::unique_lock <std::mutex> genTxMutexLock(genTxMutex);
    genTxMutexLock.unlock();
    ...
    genCcVoltageRange = ccVoltageRangesArray[CCVoltageRange1000mV];
    genVcCurrentRange = vcCurrentRangesArray[VCCurrentRange10nA];
    genCcCurrentRange = ccCurrentRangesArray[CCCurrentRange8nA];
    genSamplingRate = samplingRate;
    ...
    while (!stopConnectionFlag) {
        ...
        while (!stopConnectionFlag && protocolOpened) {
            ...
            uint16_t acquiredVValue;
            uint16_t acquiredIValue;
            ...
            case ProtocolItemVRamp:
                for (itemStepsIdx = 0; (itemStepsIdx < itemStepsNum) && (itemStepsIdx < integrationItemStepsNum); itemStepsIdx++) {
                    acquiredVValue = (uint16_t)(32768.0+(((double)(itemStepsInt+itemStepsIdx))*genVoltageStep+genVoltage)*genVoltageNorm);
                    tempValue = (((double)(itemStepsInt+itemStepsIdx))*genCurrentStep+genCurrent+genVHoldTuner/genResistance)*genCurrentNorm;
                    acquiredIValue = (uint16_t)(32768.0+this->checkSaturation(tempValue));
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredIValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredIValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredIValue;
            ...
            case ProtocolItemVSin:
                for (itemStepsIdx = 0; (itemStepsIdx < itemStepsNum) && (itemStepsIdx < integrationItemStepsNum); itemStepsIdx++) {
                    acquiredVValue = (uint16_t)(32768.0+(sin(((double)(itemStepsInt+itemStepsIdx))*genVoltageStep)*genVoltageAmp+genVoltage)*genVoltageNorm);
            ...
            case ProtocolItemIStepTStep:
                ...
                for (itemStepsIdx = 0; (itemStepsIdx < itemStepsNum) && (itemStepsIdx < integrationItemStepsNum); itemStepsIdx++) {
                    ...
                    acquiredVValue = (uint16_t)(32768.0+this->checkSaturation(tempValue));
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = genCurrentInt;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = genCurrentInt;
            ...
            case ProtocolItemIRamp:
                for (itemStepsIdx = 0; (itemStepsIdx < itemStepsNum) && (itemStepsIdx < integrationItemStepsNum); itemStepsIdx++) {
                    ...
                    tempValue = genVoltage*genVoltageNorm;
                    acquiredVValue = (uint16_t)(32768.0+this->checkSaturation(tempValue));
                    acquiredIValue = (uint16_t)(32768.0+temp2Value*genCurrentNorm);
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredIValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredVValue;
                    rxDataBuffer[(rxDataBufferWriteOffset+rxDataBufferWriteIdx++)&EZP_RX_DATA_BUFFER_MASK] = acquiredIValue;
...
double EZPatchFakeP8::checkSaturation(double value) {
    if (value > 32767.0) {
        value = 32767.0;
        saturationFlag = true;
...
void EZPatchFakeP8::integrateFHNModel(double current) {
